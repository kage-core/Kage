...
// The ground-truth anchors behind a claim. Each anchor links to the SOURCE record (a file path,
// symbol, and line range, or a source URI) so a reader can verify the claim against the repository
// itself — the honesty gate for "evidence links resolve". We render:
//   - a link whose accessible name is the human location (path + line range, falling back to the
//     source URI) pointing at the anchor's source,
    ...
    <ul className="evidence-list">
      {evidence.map((e) => (
        <li key={e.evidence_id} className="evidence-item" data-stance={e.stance}>
          <a className="evidence-link" href={e.source_uri}>
            {anchorLabel(e)}
          </a>
