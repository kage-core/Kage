...
<img src="../docs/kage-viewer.jpg" alt="kage viewer 的记忆↔代码图谱：记忆 packet 连到其依据的代码文件" width="760">

<sub>`kage viewer` 中的记忆 ↔ 代码图谱：每个记忆 packet（紫色）连到它所依据的文件（蓝色）。</sub>

你的编码代理每次会话都会忘记代码库，于是你不得不一遍遍重新解释。
**Kage** 为它提供持久记忆，以纯文本文件的形式存放在你的仓库里，并把每一条记忆
与你的真实代码对照核验，因此代理永远不会基于已经失效的内容行动。通过 git 与
整个团队共享。无需账号、无需数据库、无需 API key。
...
- **它原生于 git。** 记忆是仓库里的纯文本文件，与代码在同一个 PR 中评审，
  通过 git 与整个团队共享，而不是锁在某一台机器或某个厂商的云里。
