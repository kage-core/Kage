...
**Status:** COMPLETE — all 4 tasks done, reviewed, hardened. Anthropic + OpenAI + Gemini gateways behind one seam; per-provider honest reporting. Full suite 735/735, dogfood 12/12. New workstream, parallel to the Phase B–E program (which covers the knowledge model, portal, and commercial rollout, NOT provider breadth).
...
3. **Per-provider usage semantics must be encoded correctly.** This is the central risk. The receipt's before/after token counts must be the SAME quantity on both sides, and that quantity is the TOTAL prompt tokens including cache:
   ...
   **Verified 2026-07-15 (sources):** OpenAI `prompt_tokens` includes cached (`prompt_tokens_details.cached_tokens` is a subset) — https://developers.openai.com/api/docs/guides/prompt-caching. Gemini `promptTokenCount` includes cached (`cachedContentTokenCount` is a subset) — https://ai.google.dev/api/tokens. So for BOTH, `total_prompt_tokens` is the reported prompt count directly (no summation); the cached split only feeds pricing. Gemini streaming: `usageMetadata` appears only on the final SSE chunk, and `cachedContentTokenCount` is ABSENT (not 0) on a cache miss — treat falsy as 0 for pricing, never crash on undefined.

   **Priority (from execution review):** injection + capture + fail-open are the must-work core of each adapter — that is what makes Kage actually work for that provider's agents. Measurement must be HONEST but may be `partial`/`unavailable` wherever certainty is lacking; do NOT gold-plate exhaustive pricing. Unknown model / uncertain rate → `null` cost, never a fabricated one. Scope: OpenAI `chat/completions` + `responses`; Gemini `generateContent` + `streamGenerateContent`. **Azure OpenAI is explicitly out of scope** (different host/auth path) — a clean follow-up, not a gap.
...
### Task 2 — OpenAI-compatible provider adapter — DONE `71b689d`+`63d07f5`
...
### Task 3 — Gemini provider adapter — DONE `14446bd`+`11212c1`
...
### Task 4 — Multi-provider gate + report — DONE `c1b3898`+`7e8c69b`
